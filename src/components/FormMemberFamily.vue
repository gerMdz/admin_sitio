<template>
  <Dialog :visible="visible" @update:visible="val => emit('update:visible', val)" :header="titulo" modal class="w-50rem"
          @hide="onHide">
    <form @submit.prevent="guardar">
      <Dropdown v-model="form.member" :options="comboMembers" optionLabel="label" optionValue="id" placeholder="Miembro"
                class="mb-3 w-full"/>
      <AutoComplete
          v-model="form.family"
      ...
      <Dropdown v-model="form.related" :options="comboMembers" optionLabel="label" optionValue="id"
                placeholder="Familiar" class="mb-3 w-full"/>
      <div class="mb-3">
        <label>
          <Checkbox v-model="form.asist_church" :binary="true"/>
          Asiste a la Iglesia</label>
      </div>
      <div class="mb-3">
        <label>
          <Checkbox v-model="form.coexists" :binary="true"/>
          Convive</label>
      </div>
...
import {getComboFamilies} from '@/services/combo.service'
...
async function buscarFamily(event) {
  const res = await getComboFamilies(event.query)
